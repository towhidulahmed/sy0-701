...
import type { UnixSecurityQuestion } from "@/lib/unix-security-questions";

type Props = {
  topicTitle: string;
  topicSlug: string;
  domainName: string;
  questions: UnixSecurityQuestion[];
};

export function UnixSecurityPracticeRunner({ topicTitle, topicSlug, domainName, questions }: Props) {
    ...
    return (
      <div className="px-3 pt-4 sm:px-0">
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 text-center">
          ...
          <Link href="/unix-security/practice" className="mt-4 inline-block text-sm text-blue-400 underline">
            Back to topics
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-[calc(100dvh-6rem)] flex-col px-3 pt-4 sm:min-h-0 sm:px-0">
      ...
      <div className="mb-3 flex items-center justify-between">
        <Link href="/unix-security/practice" className="flex items-center gap-1.5 text-sm text-zinc-400 transition-colors hover:text-zinc-200">
      ...
      {isFinished && currentIdx === questions.length - 1 ? (
          ...
          <div className="w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-center sm:rounded-xl">
            ...
            <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-center">
              <button
                ...
                className="rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-500 active:bg-blue-700"
              >
                Try Again
              </button>
              <Link
        ...
        <div className="flex flex-1 flex-col rounded-2xl border border-zinc-800 bg-zinc-900 p-5 sm:rounded-xl sm:p-8">
          ...
          {answered && !isCorrect && currentIdx < questions.length - 1 && (
            <button
              onClick={handleNext}
              className="mt-4 w-full rounded-xl bg-blue-600 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-blue-500 active:bg-blue-700"
            >
              Next Question
            </button>
          )}
